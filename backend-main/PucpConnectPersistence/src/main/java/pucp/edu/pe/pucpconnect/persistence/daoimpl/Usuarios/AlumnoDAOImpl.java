...
public class AlumnoDAOImpl extends BaseDAOImpl<Alumno> implements AlumnoDAO {
    ...
    protected PreparedStatement getSelectAllPS(Connection conn) throws SQLException {
        String query = "SELECT u.idUsuario, u.nombre, u.email, u.estado, u.fecha_Registro, " +
 "                u.password, a.idAlumno, a.edad, a.carrera, a.fotoPerfil, a.ubicacion, a.biografia" +
"                FROM Usuario u LEFT JOIN  Alumno a ON u.idUsuario = a.idUsuario" +
"                ORDER BY u.fecha_Registro ASC";
    ...
    @Override
    public Map<String, Integer> obtenerEstadisticasCarreras() {
        ...
        try (Connection conn = DBManager.getInstance().obtenerConexion(); PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {

            while (rs.next()) {
                estadisticas.put(rs.getString("carrera"), rs.getInt("total"));
            }
        } catch (SQLException e) {
            throw new RuntimeException("Error al obtener estadísticas de carreras", e);
        }
