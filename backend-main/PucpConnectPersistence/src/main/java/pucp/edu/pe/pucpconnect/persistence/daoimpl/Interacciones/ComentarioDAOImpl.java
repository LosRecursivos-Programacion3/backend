...
public class ComentarioDAOImpl extends BaseDAOImpl<Comentario> implements ComentarioDAO {
    ...
    protected PreparedStatement getInsertPS(Connection conn, Comentario comentario) throws SQLException {
        String query = "{CALL comentarPost(?, ?, ?)}"; // Procedimiento almacenado para insertar comentario
        CallableStatement cs = conn.prepareCall(query);
        cs.setInt(2, comentario.getAutor().getIdAlumno());
        cs.setInt(3, comentario.getPost().getIdPost());
    ...
    protected Comentario createFromResultSet(ResultSet rs) throws SQLException {
        ...
        comentario.setFechaComentario(rs.getTimestamp("fechaComentario").toLocalDateTime());
    ...
    public List<Comentario> obtenerComentariosPorPost(int postId) {
        ...
        try (Connection conn = DBManager.getInstance().obtenerConexion()) {
            String query = "SELECT * FROM Comentario WHERE post_id = ?"; // Procedimiento almacenado para obtener comentarios por post
            CallableStatement cs = conn.prepareCall(query);
            cs.setInt(1, postId);
